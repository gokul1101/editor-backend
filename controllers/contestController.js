...
const getContest = async (req, res) => {
  ...
  try {
    ...
    if (!contest) return res.status(404).json({message:`Contest not found`,success:false});
    if (req.user.role === "student") {
      ...
      if (now > end_date)
        return res.status(403).json({message:`The contest was expired.`,success:false});
      //* If the contest is not started yet.
      if (now < start_date)
        return res.status(403).json({message:`The contest is not started yet.`,success:false});
    }
